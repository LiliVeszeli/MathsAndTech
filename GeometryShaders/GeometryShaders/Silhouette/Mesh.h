...
public:
    ...
    Mesh(const std::string& fileName, bool requireTangents = false, bool requireAdjacency = false);
    ~Mesh();
...
private:
	...
	// Given a indices of a triangle list calculate the adjacency indices (as required for geometry shaders using triangle lists with adjacency.
	// Although per-vertex data is flexible the first data for every vertex is 3 floats for position, x, y, z. That's all that is used here
	// Will consider vertices within given snap range as the same vertex here
	std::unique_ptr<uint32_t[]> CalculateAdjacency(unsigned char* vertices, unsigned int vertexSize, unsigned int numVertices, 
...
private:
	...
	bool mHasAdjacency; // Flag if this mesh has adjacency data (for geomeetry shaders)
};
