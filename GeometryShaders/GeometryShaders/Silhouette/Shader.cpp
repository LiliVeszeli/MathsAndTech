...
ID3D11GeometryShader* gSilhouetteGeometryShader     = nullptr; // New geometry shader stage - create them in the same way as other shaders
...
bool LoadShaders()
{
	...
	gSilhouetteGeometryShader       = LoadGeometryShader("Silhouette_gs"); // Load geometry shader file

	if (gPixelLightingVertexShader  == nullptr || gPixelLightingPixelShader == nullptr ||
		gBasicTransformVertexShader == nullptr || gTintedTexturePixelShader == nullptr || gSilhouetteGeometryShader == nullptr)
	{
...
void ReleaseShaders()
{
	if (gSilhouetteGeometryShader)        gSilhouetteGeometryShader      ->Release();
