...
bool InitGeometry()
{
	...
	if (!LoadTexture("Moon.jpg", &gSphereDiffuseSpecularMap, &gSphereDiffuseSpecularMapSRV) ||
...
bool InitScene()
{
	...
	// Initial positions
	gSphere->SetPosition({ 20, 20, 0 });
    gSphere->SetScale(1.0f);
...
void RenderSceneFromCamera(Camera* camera)
{
	...
	gD3DContext->GSSetShader(gExplodeGeometryShader, nullptr, 0);          ///// Enable geometry shader
    ...
    gD3DContext->PSSetShaderResources(0, 1, &gSphereDiffuseSpecularMapSRV); // First parameter must match texture slot number in the shader
    gD3DContext->PSSetSamplers(0, 1, &gAnisotropic4xSampler);

	//// Explosion factor used in the geometry shader
	gPerModelConstants.explodeAmount = 8.0f;
...
void UpdateScene(float frameTime)
{
	...
	gSphere->Control(0, frameTime, Key_I, Key_K, Key_J, Key_L, Key_U, Key_O, Key_Period, Key_Comma );
    ...
    if (totalFrameTime > fpsUpdateTime)
    {
        ...
        std::string windowTitle = "CO3303 Week 7: Geometry Shaders 1 - Frame Time: " + frameTimeMs.str() +
