...
ID3D11GeometryShader* gExplodeGeometryShader      = nullptr; // New geometry shader stage - create them in the same way as other shaders
...
bool LoadShaders()
{
	...
	gExplodeGeometryShader          = LoadGeometryShader("Explode_gs"); // Load geometry shader file

	if (gPixelLightingVertexShader  == nullptr || gPixelLightingPixelShader == nullptr ||
		gBasicTransformVertexShader == nullptr || gTintedTexturePixelShader == nullptr || gExplodeGeometryShader == nullptr)
	{
...
void ReleaseShaders()
{
	if (gExplodeGeometryShader)           gExplodeGeometryShader         ->Release();
