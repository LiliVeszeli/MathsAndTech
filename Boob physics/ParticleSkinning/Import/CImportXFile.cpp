...
(
	...
	TUInt32       numSkinningParticles /*= 0*/,
	CVector3*     skinningParticles /*= 0*/,
	TFloat32      skinningDist /*= 0.5f*/
) const
{
	GEN_GUARD;
	...
	pOutSubMesh->hasSkinningData = (m_Meshes[iSubMesh].bones.size() > 0 || numSkinningParticles > 0);
	...
	// Calculate bone or particle influences if necessary
	if (pOutSubMesh->hasSkinningData)
	{
		...
		//****| INFO | Create index/weights for particle system skinned model when loading ****//
		if (numSkinningParticles > 0) 
		{
			// Find skinning particle influences - nearest four are chosen
			TUInt8* pVert = pOutSubMesh->vertices;
			for (TUInt32 vert = 0; vert < pOutSubMesh->numVertices; ++vert)
			{
				CVector3& v = *reinterpret_cast<CVector3*>(pVert);
				...
				for (TUInt32 part = 0; part < numSkinningParticles; ++part)
				{
					CVector3& p = skinningParticles[part];
					TFloat32 dist = Distance( v, p );
					if (dist < skinningDist)
					{
						//****| INFO |****//
						// This formula makes the weights of vertices around a particle fall off as distance increases. If the vertex is on top of the
						// particle then the weight will be 1.0. The weight will fall off to 0 at the distance "skinningDist", a parameter to the import
						// function. The powf function gives a curve to the fall off, which can be tweaked
						TFloat32 weight = powf((skinningDist - dist) / skinningDist, 1.0f); 
						AddBoneInfluence( part, weight, pWeights, pInfluences );
					}
				}
				// Sum of weights can be less than 1 (some areas are not influenced), but not greater than 1
				float sum = pWeights[0] + pWeights[1] + pWeights[2] + pWeights[3];
				if (sum > 1.0f)
				{
					pWeights[0] /= sum;
					pWeights[1] /= sum;
		...
		else // Standard skinning with bones
		{
			// For each bone...
