...
// Can optionally pass a skinning particle system which will be used to create vertex weights
...
bool CModel::Load( const string& fileName, ID3D10EffectTechnique* exampleTechnique, bool tangents /*= false*/, int numSkinningParticles /*= 0*/, gen::CVector3* skinningParticles /*= 0*/, float skinningDist /*= 0.5f*/ )
{
	...
	if (mesh.GetSubMesh( 0, &subMesh, tangents, numSkinningParticles, skinningParticles, skinningDist ) != gen::kSuccess)
	{
		return false;
	}
	...
	if (subMesh.hasSkinningData) // If sub-mesh contains skinning data (from particles in this lab)
	{
