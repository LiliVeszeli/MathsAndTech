...
ID3D11VertexShader* gTessellationVertexShader = nullptr;
ID3D11HullShader*   gTessellationHullShader   = nullptr;
ID3D11DomainShader* gTessellationDomainShader = nullptr;
ID3D11PixelShader*  gTessellationPixelShader  = nullptr;
...
bool LoadShaders()
{
	...
	gTessellationVertexShader     = LoadVertexShader  ("Tessellation_vs"   );
	gTessellationHullShader       = LoadHullShader    ("Tessellation_hs"   );
	gTessellationDomainShader     = LoadDomainShader  ("Tessellation_ds"   );
	gTessellationPixelShader      = LoadPixelShader   ("Tessellation_ps"   );
...
void ReleaseShaders()
{
	...
	if (gTessellationVertexShader)  gTessellationVertexShader->Release();
	if (gTessellationHullShader  )  gTessellationHullShader  ->Release();
	if (gTessellationDomainShader)  gTessellationDomainShader->Release();
	if (gTessellationPixelShader )  gTessellationPixelShader ->Release();
...
// Load a hull shader, include the file in the project and pass the name (without the .hlsl extension)
// to this function. The returned pointer needs to be released before quitting. Returns nullptr on failure. 
ID3D11HullShader* LoadHullShader(std::string shaderName)
{
	...
	HRESULT hr = gD3DDevice->CreateHullShader(byteCode.data(), byteCode.size(), nullptr, &shader);
	if (FAILED(hr))
	{
		return nullptr;
	}
...
// Load a domain shader, include the file in the project and pass the name (without the .hlsl extension)
// to this function. The returned pointer needs to be released before quitting. Returns nullptr on failure. 
ID3D11DomainShader* LoadDomainShader(std::string shaderName)
{
	...
	HRESULT hr = gD3DDevice->CreateDomainShader(byteCode.data(), byteCode.size(), nullptr, &shader);
	if (FAILED(hr))
	{
		return nullptr;
	}
