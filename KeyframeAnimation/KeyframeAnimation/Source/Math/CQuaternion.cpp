...
)
{
	...
	//****MISSING - use lecture notes to add correct formula
	qt = q0 * (1 - t) + q1 * t;
}
...
{
	// Simple linear interpolation equation
	//****MISSING - exactly the same as the formula added in the Lerp function above
	qt = q0 * (1 - t) + q1 * t;
...
)
{
	...
	if (cosTheta >= 0.0f)
	{
		...
		if (!AreEqual( cosTheta, 1.0f ))
		{
			...
			slerp = (sin((1 - t) * theta) * p + sin(t * theta) * q) / sin(theta);
		}
		else
		{
			// Small angle - lerp calculation is better
			//****MISSING - same as the formula added in the Lerp function above
			slerp = p * (1 - t) + q * t;
		}
	}
	else
	{
		...
		if (!AreEqual( cosTheta, -1.0f ))
		{
			...
			slerp = (sin(( t - 1) * theta) * p + sin(t * theta) * q) / sin(theta);
		}
		else
		{
			// Small angle - lerp calculation is better, but use (t-1) instead of (1-t)
			//****MISSING - similar to the formula added in the Lerp function above
			slerp = p * (t - 1) + q * t;

			//OK LOOKING AT IT NOW, JUST A MOMENT
		}
	}
}
