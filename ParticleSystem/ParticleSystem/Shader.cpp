...
ID3D11VertexShader* gWorldTransformOnlyVertexShader = nullptr;
...
ID3D11GeometryShader* gParticleRenderShader = nullptr; // Geometry shader used for rendering particles
ID3D11GeometryShader* gParticleUpdateShader = nullptr; // Geometry shader used for updating particles
ID3D11VertexShader*   gParticlePassThruVertexShader = nullptr; // Vertex shader does nothing when rendering and updating particles
...
bool LoadShaders()
{
	...
	gWorldTransformOnlyVertexShader = LoadVertexShader("WorldTransformOnly_vs");
	...
	gParticleRenderShader         = LoadGeometryShader("ParticleRender_gs");
	gParticlePassThruVertexShader = LoadVertexShader  ("ParticlePassThru_vs");

	if (gPixelLightingVertexShader  == nullptr || gPixelLightingPixelShader == nullptr ||
		gBasicTransformVertexShader == nullptr || gTintedTexturePixelShader == nullptr ||
		gParticleRenderShader       == nullptr || gParticlePassThruVertexShader == nullptr)
	{
...
void ReleaseShaders()
{
	if (gParticlePassThruVertexShader)    gParticlePassThruVertexShader  ->Release();
	if (gParticleUpdateShader)            gParticleUpdateShader          ->Release();
	if (gParticleRenderShader)            gParticleRenderShader          ->Release();
	...
	if (gWorldTransformOnlyVertexShader)  gWorldTransformOnlyVertexShader->Release();
