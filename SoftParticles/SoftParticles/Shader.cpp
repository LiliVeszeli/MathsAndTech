...
ID3D11VertexShader*   gParticlePassThruVertexShader = nullptr;
ID3D11GeometryShader* gParticleGeometryShader       = nullptr; // Geometry shader used to expand particle points into quads
ID3D11PixelShader*    gSoftParticlePixelShader      = nullptr; // Special pixel shader for soft particles - unlike other pixel shaders it works with the depth values as well as colours
...
bool LoadShaders()
{
	...
	gParticleGeometryShader       = LoadGeometryShader("ParticleRender_gs"  );
	gSoftParticlePixelShader      = LoadPixelShader   ("SoftParticle_ps"    );
	gTintedTexturePixelShader     = LoadPixelShader   ("TintedTexture_ps"   );
	gPixelLightingPixelShader     = LoadPixelShader   ("PixelLighting_ps"   );


	if (gBasicTransformVertexShader   == nullptr || gPixelLightingPixelShader == nullptr ||
		gParticlePassThruVertexShader == nullptr || gParticleGeometryShader   == nullptr ||
		gSoftParticlePixelShader      == nullptr || gTintedTexturePixelShader == nullptr ||
...
void ReleaseShaders()
{
	...
	if (gSoftParticlePixelShader)       gSoftParticlePixelShader     ->Release();
	if (gParticleGeometryShader)        gParticleGeometryShader      ->Release();
